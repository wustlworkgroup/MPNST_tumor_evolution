...
Data_select=df%>%dplyr::filter(Norm_RDR>1.1&Norm_RDR<1.45&BAF>0.3&BAF<0.47)
...
if (grepl("PDX",Sample_name)){
 ...
 Data_select=df%>%dplyr::filter(Norm_RDR>1.1&Norm_RDR<1.5&BAF>0.3&BAF<0.47)
...
results <- Data_select%>%.[,c("Chromosome","Start", "End","BAF","Norm_RDR")]%>%dplyr::rename(RDR=Norm_RDR) %>%
  rowwise() %>%
  mutate(
    Analysis = list(Trisomy_model_predict(trisomy_model , RDR, BAF, threshold = 0.02))
  )%>%ungroup() %>%  # Optionally remove old columns if you only want the results
  tidyr::unnest_wider(col = Analysis)%>%arrange(RDR)

Data_select_2=Data_select%>%left_join(.,results[,c(1,2,3,9)])%>%mutate(CNV=ifelse(Is_Close_To_Line==T,"Trisomy","Other"))
G1=ggplot(Data_select_2,aes(x=BAF,y=Norm_RDR,color=factor(Is_Close_To_Line)))+geom_point()+geom_point(data=trisomy_data,aes(x=BAF,y=RDR),shape=23, fill="blue", color="darkred", size=3)


coef_intercept <- coef(trisomy_model)[1]
coef_slope <- coef(trisomy_model)[2]
...
G2=ggplot(Data_select_3 ,aes(x=BAF,y=Norm_RDR,color=factor(CNV2)))+geom_point()+geom_point(data=trisomy_data,aes(x=BAF,y=RDR),shape=23, fill="blue", color="darkred", size=3)
...
categorized_data<- categorize_CNV(df)%>%left_join(.,Data_select_3[,c("Chromosome","Start", "End","CNV2")])%>%mutate(CNV = ifelse(is.na(CNV2), CNV, ifelse(CNV2 == "Trisomy", "Trisomy", CNV)))
