...
```{r}
# The data from step1
load(file.path(Process_path,paste0(PT_ID,"_Germline_somatic_RD_BAF.rdata")))
```
...
Somatic_CN_plot_2 <- function(Somatic_SV, Germline_SV = NULL,MAX_CN=4) {
...
#    kpPlotRegions(kp, Somatic_SV_torange, col = Somatic_SV_select$line_color, border = Somatic_SV_select$line_color, r0 =  (Somatic_SV_select$CN/MAX_CN)-0.05, r1 = Somatic_SV_select$CN/MAX_CN)
  ...
  title(main = "Somatic CNV Plot", cex.main = 1.5, line = 2)
...
```{r,include=F}
Data_filter=Somatic_CNV[[Sample_name]]%>%Gene_annotation_for_CNV_2(.)
```
...
Out=density_with_hdi (Data_filter$RDR,bandwidth = 0.02, credMass = 0.9)

# if the main peak is not located around 1, that means read depth has bias.
Max_out=Out[[2]]%>%dplyr::filter(Y==max(Y))

Bias_range=c(Max_out$Value-0.01,Max_out$Value+0.01)
...
# Need adjust cluster number in yml based on the variety of data.
 Cluster_number=config$genomic_settings$Cluster_number
...
normal_setting <- config$genomic_settings$normal
LOH_setting <- config$genomic_settings$LOH
trisomy_setting <- config$genomic_settings$trisomy
tetrasomy_setting <- config$genomic_settings$tetrasomy
tetrasomy_LOH_setting <- config$genomic_settings$tetrasomy_LOH
het_loss_setting <- config$genomic_settings$het_loss
homo_loss_setting <- config$genomic_settings$homo_loss
...
# Example usage
 set.seed(12345)
 Bandwidth=config$Clonality_density$bandwith_1
...
Bandwidth=config$Clonality_density$bandwith_2
...
if(nrow(High_ampli_data_baf_0.5) >0){
 ...
 High_ampli_data2=High_ampli_data_baf_0.5%>%mutate(CN=Test$CN,Group2=Test$Group,Value=Test$Value)
...
Data_update_10_filter=Data_update_10%>%dplyr::filter(!grepl(">4",as.character(CN)))%>%mutate(CN=as.numeric(CN))

MAX_cn=max(Data_update_10_filter$CN)
Somatic_CN_plot_2(Data_update_10_filter,Germline_SV =Germline_CNV_ALT_unique,MAX_CN =MAX_cn)
...
knitr::kable(NF1,caption = "NF1 somatic",booktabs = TRUE) %>%kableExtra::kable_styling(latex_options =  c("scale_down","hold_position"))%>%kableExtra::column_spec(column=2, width = "3cm")
...
knitr::kable(CDKN2A,caption = "CDKN2A somatic",booktabs = TRUE) %>%kableExtra::kable_styling(latex_options =  c("scale_down","hold_position"))%>%kableExtra::column_spec(column=2, width = "3cm")
...
save(Data_update_10,file=file.path(Process_path,PT_ID ,paste0(PT_ID,"_",Sample_name,"_CNV_clonality.rdata")))
```
