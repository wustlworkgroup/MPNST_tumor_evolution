...
```{r,include=F}
# define sample information and file location
PT_ID="JH-2-051"
# setup your personal path
...
Process_path=file.path(Path,"Process")
WEX_rawdata_path=file.path(Rawdata_path,"WEX")
...
synapser::synLogin(email="",password = "")
...
processGermlineCNV <- function(Patient_ID, Process_Path, Sample_anno) {
  ...
  for (i in Germline_CNV_combine$synid) {
    ...
    synapser::synGet(i, downloadLocation = Download_path)
...
process_somatic_CNV <- function(Patient_ID, Process_path) {
  ...
  for (i in Samples) {
    ...
    for (s in synID) {
      synapser::synGet(s, downloadLocation = Download_path)
    }
...
if (!file.exists(file.path(Rawdata_path,annotation_filename))){
 synapser::synGet(Sample_anno_syn,downloadLocation =Rawdata_path)
...
if (!file.exists(file.path(Rawdata_path,Specimen_ID_filefname))){

  synapser::synGet(specimen_id_syn,downloadLocation =Rawdata_path)
...
if (! file.exists(file.path(Process_path,paste0(PT_ID,"_Germline_somatic_RD_BAF.rdata")))) {
  ...
  save(Germline_CNV,Somatic_CNV,file=file.path(Process_path,paste0(PT_ID,"_Germline_somatic_RD_BAF.rdata")))
}
```
