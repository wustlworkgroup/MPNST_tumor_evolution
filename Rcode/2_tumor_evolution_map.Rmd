---
title: "`r paste0(config::get(file = 'Step2_config.yml')$PT_ID, '_Tumor_evolution_analysis')`"
...
knit: (
  function(inputFile, encoding) {
    ...
    pSubTitle <- paste0(config$PT_ID,"_",config$Sample_input,"_Tumor_evolution.pdf")
...
# The data from step1
dir(Process_path)
Clonality_data=list.files(file.path(Process_path,PT_ID),pattern ="CNV_clonality.rdata",full.names = T,recursive = T )
MPNST_data_pickup=Clonality_data[c(5,4,2)]
...
for (i in 1:length(MPNST_data_pickup)) {
 file=MPNST_data_pickup[i]
 load(file)  
 MAX_cn=4
...
```{r,fig.width=12,fig.height=16,include=T,eval=F}

cowplot::plot_grid(plotlist =CN_plotlist,ncol=1)
```
