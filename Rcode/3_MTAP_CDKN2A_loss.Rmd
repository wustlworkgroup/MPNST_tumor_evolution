...
if (!file.exists(file.path(Process_path,PT_ID,filename) )){
 ...
 RD_data=readr::read_tsv(Tumor_RD_path)%>%as.data.frame()%>%setNames(c("CHR","Other"))%>%dplyr::filter(CHR!="@SQ")%>%dplyr::filter(CHR!="@RG")%>%
  ...
  BAF_path=list.files(Data_path,pattern = "Counts_cleaned.tsv",full.names = T,recursive = T)%>%.[grepl(Sample_name,.)]

  BAF_data=readr::read_tsv(BAF_path)%>%dplyr::filter(CONTIG==nchr)%>%dplyr::rename(CHR=CONTIG)
 
  BAF_data_anno=add_gene_names_to_BAF(BAF_data)

  save(RD_data_anno,BAF_data_anno,file=file.path(Process_path,PT_ID,filename))
} 
...
BAF_data_anno2=BAF_data_anno%>%mutate(baf=alt_count/(ref_count+alt_count))%>%mutate(baf=ifelse(baf>0.5,1-baf,baf))
...
gene_baf_level=summarize_baf_by_gene(BAF_data_anno2)
gene_baf_rdr=gene_rdr_level%>%left_join(.,gene_baf_level)%>%as.data.frame()

BAF_data_anno3=BAF_data_anno2%>%mutate(start=position,end=position)

segment_results_by_rdr=detect_segments_in_rdr(RD_data_anno_norm,threshold = 0.2, min_segment_size = 8)
segment_results_by_gene <- detect_segments_in_rdr_by_gene (gene_rdr_level2)
# Plot detected segments on chromosome 1

rdr_data=segment_results_by_gene[[1]]%>%dplyr::rename(rdr=mean_rdr)
Choose_Seg=rdr_data%>%dplyr::filter(grepl("MTAP|CDKN2A",genes))

Choose_Seg_2=Choose_Seg[,c(1:3)]%>%mutate(start=min(start),end=max(end))%>%distinct()


Gene_list_in_region=Choose_Seg$genes%>%strsplit(.,",")%>%unlist()%>%.[!grepl("unknown",.)]

Genes_Data=gene_baf_rdr%>%dplyr::filter(gene_name%in%Gene_list_in_region)%>%arrange(start)
```
...
if (file.exists(file.path(Process_path,PT_ID,"MTAP_CDKN2A",paste0(Sample_name,"_gene_level_Seg.rdata")))) {
  ...
  segment_plot_detail <-  ggplotify::as.ggplot(function() {plot_detected_segments (RD_data_anno_norm,rdr_data,gene_baf_rdr,Gene_chorom=Choose_Seg_2,plot_zoom = T)})
 segment_plot <-  ggplotify::as.ggplot(function() {plot_detected_segments (RD_data_anno_norm,rdr_data,gene_baf_rdr,Gene_chorom=Choose_Seg_2,plot_zoom = F)})
 save(gene_rdr_level2,gene_baf_level,gene_baf_rdr,segment_results_by_gene,Choose_Seg,Genes_Data,segment_plot,segment_plot_detail,file=file.path(Process_path,PT_ID,"MTAP_CDKN2A",paste0(Sample_name,"_gene_level_Seg.rdata")))
...
```{r,include=T}
knitr::kable(Choose_Seg,caption =   "Copy number variant in segments")%>%kableExtra::kable_styling(latex_options = c("scale_down"))

knitr::kable(Genes_Data,caption = "Copy number variant in genes")%>%kableExtra::kable_styling(latex_options = c("scale_down"))
```
...
```{r,include=T}
knitr::kable(Gene_GATK_seg[,c(1,2,3,4,8,13,14,9)],caption = "GATK segmentation",full_width=F)%>%
  kableExtra::column_spec(8, width = "5cm")%>%
  kableExtra::column_spec(1, width = "2cm")%>%kableExtra::kable_styling(latex_options = c("scale_down"))
```


```{r}
Gene_baf=BAF_data_anno2%>%dplyr::filter(gene_name%in%c("MTAP","CDKN2A","CDKN2B"))
if (nrow (Gene_baf) >0) {
