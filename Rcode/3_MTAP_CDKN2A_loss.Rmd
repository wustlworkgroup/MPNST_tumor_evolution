...
load(file.path(Process_path,PT_ID,"MTAP_CDKN2A",filename))
RD_data_anno_norm=RD_data_anno%>%mutate(rdr=rdr/Bias_set) #Bias_set is from step2
#BAF_data_anno
 
BAF_data_2=BAF_data%>%mutate(baf=alt_count/(ref_count+alt_count))%>%dplyr::filter(alt_nucleotide!="N")%>%dplyr::filter(alt_count>2)%>%mutate(baf=ifelse(baf>0.5,1-baf,baf))%>%mutate(start=position,end=position)
...
if (nrow (Gene_rdr) >0) {
  ...
  knitr::kable(Gene_rdr,caption = "rdr in bed")%>%kableExtra::kable_styling(latex_options = c("scale_down"))
